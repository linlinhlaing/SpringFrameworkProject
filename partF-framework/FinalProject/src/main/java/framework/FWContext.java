...
public class FWContext {
    ...
    private void performDI() {
        try {
            for (Object theServiceClass : serviceObjectList) {
                ...
                profileInjection(theServiceClass);
            ...
            if(method.isAnnotationPresent(Scheduled.class)) {
                ...
                for(Annotation annotation:annotations) {
                    if(annotation.annotationType().getName().contentEquals(Scheduled.class.getName())) {
                        int scheduledValue = ((Scheduled) annotation).fixedRate();
                        Timer timer = new Timer();
                        TimerTask task = getTimerTask(theServiceClass, method);
                        timer.scheduleAtFixedRate(task,0, scheduledValue);
                    }
                }
            }
        }
    ...
    private void profileInjection(Object theServiceClass) {
